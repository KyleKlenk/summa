...
contains
 ...
 select case(ix_canopySrad)
  ...
  case(CLM_2stream,UEB_2stream,NL_scatter,BeersLaw)
   ...
   call canopy_SW(&
                  ...
                  spectralAlbGndDirect,                               & ! intent(out): direct  albedo of underlying surface (1:nSpecBand) (-)
                  spectralAlbGndDiffuse,                              & ! intent(out): diffuse albedo of underlying surface (1:nSpecBand) (-)
 ...
 real(rkind),intent(out)           :: spectralBelowCanopyDiffuse(:)             ! downward diffuse flux below veg layer (W m-2)
 real(rkind),intent(out)           :: scalarBelowCanopySolar                    ! radiation transmitted below the canopy (W m-2)
 real(rkind),intent(out)           :: spectralAlbGndDirect(:)                   ! direct  albedo of underlying surface (1:nSpecBand) (-)
 real(rkind),intent(out)           :: spectralAlbGndDiffuse(:)                  ! diffuse albedo of underlying surface (1:nSpecBand) (-)
 real(rkind),intent(out)           :: scalarGroundAlbedo                        ! albedo of the ground surface (-)
 real(rkind),intent(out)           :: scalarCanopyAbsorbedSolar                 ! radiation absorbed by the vegetation canopy (W m-2)
 real(rkind),intent(out)           :: scalarGroundAbsorbedSolar                 ! radiation absorbed by the ground (W m-2)
 real(rkind),intent(out)           :: scalarCanopySunlitFraction                ! sunlit fraction of canopy (-)
 real(rkind),intent(out)           :: scalarCanopySunlitLAI                     ! sunlit leaf area (-)
 real(rkind),intent(out)           :: scalarCanopyShadedLAI                     ! shaded leaf area (-)
 real(rkind),intent(out)           :: scalarCanopySunlitPAR                     ! average absorbed par for sunlit leaves (w m-2)
 real(rkind),intent(out)           :: scalarCanopyShadedPAR                     ! average absorbed par for shaded leaves (w m-2)
 integer(i4b),intent(out)          :: err                                       ! error code
 character(*),intent(out)          :: message                                   ! error message
 ! -----------------------------------------------------------------------------------------------------------------------------------------------------------------
 ! local variables
 ...
 scalarBelowCanopySolar    = 0._rkind  ! radiation transmitted below the canopy (W m-2)
 scalarCanopyAbsorbedSolar = 0._rkind  ! radiation absorbed by the vegetation canopy (W m-2)
 scalarGroundAbsorbedSolar = 0._rkind  ! radiation absorbed by the ground (W m-2)
 ...
 if(.not.computeVegFlux .or. scalarCosZenith < tiny(scalarCosZenith))then
  ...
  do iBand=1,nSpecBand
   ...
   if(scalarCosZenith > tiny(scalarCosZenith))then
    ...
    spectralBelowCanopyDirect(iBand)  = 0._rkind
 ...
 select case(ix_canopySrad)
  ...
  case(BeersLaw)
   ...
   groundAlbedoDiffuse = Frad_vis*spectralAlbGndDiffuse(ixVisible) + (1._rkind - Frad_vis)*spectralAlbGndDiffuse(ixNearIR)
   ...
   do iBand=1,nSpecBand
    ...
    ! compute ground albedo (-)
    scalarGroundAlbedo  = Fdirect*groundAlbedoDirect + (1._rkind - Fdirect)*groundAlbedoDiffuse
    if(scalarGroundAlbedo < 0._rkind .or. scalarGroundAlbedo > 1._rkind)then
    ...
    spectralGroundAbsorbedDirect(iBand)  = (1._rkind - scalarGroundAlbedo)*spectralBelowCanopyDirect(iBand)
    spectralGroundAbsorbedDiffuse(iBand) = (1._rkind - scalarGroundAlbedo)*spectralBelowCanopyDiffuse(iBand)
    ...
    fracRadAbsUp   = tauTotal*scalarGroundAlbedo*(1._rkind - tauTotal)   ! (fraction of radiation absorbed on the way up)
  ...
  case(NL_scatter)
   ...
   do iBand=1,nSpecBand
    ...
    refMult = 1._rkind / (1._rkind - scalarGroundAlbedo*bulkCanopyAlbedo*(1._rkind - taudFinite**multScatExp) )
    ...
    ! compute radiation absorbed by vegetation in current wave band (W m-2)
    fracRadAbsDown = (1._rkind - tauTotal)*(1._rkind - bulkCanopyAlbedo)                            ! (fraction of radiation absorbed on the way down)
    fracRadAbsUp   = tauTotal*refMult*scalarGroundAlbedo*(1._rkind - taudFinite**multScatExp)   ! (fraction of radiation absorbed on the way up)
    spectralCanopyAbsorbedDirect(iBand)  = spectralIncomingDirect(iBand)*(fracRadAbsDown + fracRadAbsUp)
    spectralCanopyAbsorbedDiffuse(iBand) = spectralIncomingDiffuse(iBand)*(fracRadAbsDown + fracRadAbsUp)
    spectralCanopyAbsorbedSolar(iBand)   = spectralCanopyAbsorbedDirect(iBand) + spectralCanopyAbsorbedDiffuse(iBand)
  ...
  case(UEB_2stream)
   ...
   do iBand=1,nSpecBand
    ...
    ! compute radiation absorbed by the ground in given wave band (W m-2)
    spectralGroundAbsorbedDirect(iBand)  = (1._rkind - groundAlbedoDirect)*spectralBelowCanopyDirect(iBand)
    spectralGroundAbsorbedDiffuse(iBand) = (1._rkind - groundAlbedoDiffuse)*spectralBelowCanopyDiffuse(iBand)
    spectralGroundAbsorbedSolar(iBand)   = spectralGroundAbsorbedDirect(iBand) + spectralGroundAbsorbedDiffuse(iBand)

    ! compute radiation absorbed by vegetation in current wave band (W m-2)
    spectralCanopyAbsorbedDirect(iBand)  = spectralIncomingDirect(iBand)*(1._rkind - tauFinite)*(1._rkind - betaFinite) + &    ! (radiation absorbed on the way down)
                                           spectralBelowCanopyDirect(iBand)*groundAlbedoDirect*(1._rkind - taudFinite)      ! (radiation absorbed on the way up)
    spectralCanopyAbsorbedDiffuse(iBand) = spectralIncomingDiffuse(iBand)*(1._rkind - taudFinite)*(1._rkind - betadFinite) + & ! (radiation absorbed on the way down)
                                           spectralBelowCanopyDiffuse(iBand)*groundAlbedoDiffuse*(1._rkind - taudFinite)    ! (radiation absorbed on the way up)
    spectralCanopyAbsorbedSolar(iBand)   = spectralCanopyAbsorbedDirect(iBand) + spectralCanopyAbsorbedDiffuse(iBand)
  ...
  case(CLM_2stream)
   ...
   do iBand=1,nSpecBand
    ...
    call twoStream(&
                   ...
                   spectralAlbGndDirect,              & ! intent(in): direct  albedo of underlying surface (1:nSpecBand) (-)
                   spectralAlbGndDiffuse,             & ! intent(in): diffuse albedo of underlying surface (1:nSpecBand) (-)
                   ...
                   spectralDirectBelowCanopyDirect,   & ! intent(out): down dir flux below veg layer (per unit in flux), (1:nSpecBand)
                   spectralDiffuseBelowCanopyDirect,  & ! intent(out): down dif flux below veg layer (per unit in flux), (1:nSpecBand)
                   scalarGproj,                       & ! intent(out): projected leaf+stem area in solar direction
                   spectralCanopyReflectedDirect,     & ! intent(out): flux reflected by veg layer   (per unit incoming flux), (1:nSpecBand)
                   spectralGroundReflectedDirect,     & ! intent(out): flux reflected by ground (per unit incoming flux), (1:nSpecBand)
                   ! input-output
    ...
    call twoStream(&
                   ...
                   spectralDirectBelowCanopyDiffuse,  & ! intent(out): down dir flux below veg layer (per unit in flux), (1:nSpecBand)
                   spectralDiffuseBelowCanopyDiffuse, & ! intent(out): down dif flux below veg layer (per unit in flux), (1:nSpecBand)
                   scalarGproj,                       & ! intent(out): projected leaf+stem area in solar direction
                   spectralCanopyReflectedDiffuse,    & ! intent(out): flux reflected by veg layer   (per unit incoming flux), (1:nSpecBand)
                   spectralGroundReflectedDiffuse,    & ! intent(out): flux reflected by ground (per unit incoming flux), (1:nSpecBand)
                   ! input-output
                   scalarBetweenCanopyGapFraction,    & ! intent(inout): between canopy gap fraction for beam (-)
                   scalarWithinCanopyGapFraction      ) ! intent(inout): within canopy gap fraction for beam (-)
 ...
 scalarCanopySunlitFraction = (1._rkind - exp(-ext*scalarExposedVAI)) / max(ext*scalarExposedVAI,mpe)
 if(scalarCanopySunlitFraction < 0.01_rkind) scalarCanopySunlitFraction = 0._rkind

 ! compute sunlit and shaded LAI
 scalarCanopyShadedFraction = 1._rkind - scalarCanopySunlitFraction
